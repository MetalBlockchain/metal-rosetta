...
docker-build:
	docker build \
		--build-arg METAL_VERSION=${METAL_VERSION} \
...
run-testnet:
	docker run \
		...
		-d \
		-v ${WORKDIR}/data:/data \
		-e METAL_NETWORK=Tahoe \
		-e METAL_CHAIN=381932 \
		-e METAL_MODE=online \
		--name metal-testnet \
		-p 8080:8080 \
		-p 9650:9650 \
...
run-testnet-offline:
	docker run \
		--rm \
		-d \
		-e METAL_NETWORK=Fuji \
		-e METAL_CHAIN=381932 \
		-e METAL_MODE=offline \
		--name metal-testnet-offline \
		-p 8080:8080 \
		-p 9650:9650 \
		-it \
		${DOCKER_TAG}
...
run-mainnet:
	docker run \
		...
		-d \
		-v ${WORKDIR}/data:/data \
		-e METAL_NETWORK=Mainnet \
		-e METAL_CHAIN=381931 \
		-e METAL_MODE=online \
		--name metal-mainnet \
		-p 8080:8080 \
		-p 9650:9650 \
